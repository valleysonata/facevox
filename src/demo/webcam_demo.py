...
class WebcamDemo:
    ...
    @staticmethod
    def _load_classifier(model_path: Optional[str], model_type: str):
        """Load an explicit model file, or auto-load the best available checkpoint.

        Falls back to the rule-based classifier when nothing is trained yet.
        """
        candidates = []
        if model_path:
        ...
        for candidate_path, mtype in candidates:
            if os.path.exists(candidate_path):
                try:
                    clf = create_classifier(model_type=mtype)
                    clf.load(candidate_path)
        ...
        return create_classifier(model_type=model_type)
    ...
    def _process_frame(self, frame: np.ndarray) -> dict:
        ...
        adapted_landmarks, occlusion = self.occlusion_handler.process(
            landmarks_obj.landmarks, use_mask_adaptation=True,
        )
        result['occlusion'] = occlusion
        result['features'] = features
        result['annotated_frame'] = annotated

        raw_lm = normalize_landmarks(landmarks_obj.landmarks).flatten().tolist()
    ...
    def _draw_ui(self, frame: np.ndarray, result: dict, fps: float) -> np.ndarray:
        ...
        if self.show_expression and result['expression']:
            ...
            cv2.putText(display, f"Expression: {expr.label.value.upper()}", (10, 70),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.8, color, 2)
            cv2.putText(display, f"Confidence: {expr.confidence:.2f}", (10, 100),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.6, color, 1)
            sorted_probs = sorted(expr.probabilities.items(), key=lambda x: x[1], reverse=True)[:3]
            y_offset = 130
            for label, prob in sorted_probs:
                cv2.putText(display, f"{label.value}: {prob:.2f}", (10, y_offset),
        ...
        if result['occlusion'] and result['occlusion'].is_occluded:
            cv2.putText(display, f"Occlusion: {result['occlusion'].occluded_regions}",
                        (10, h - 30), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 165, 255), 1)

        cv2.putText(display, "q: quit | s: screenshot | r: reset intent",
